...
class ComponentValidator(BaseValidator):
    ...
    def validate(self, context, rules, reporter) -> None:
        ...
        if (
            ...
            and context.root_id
            and context.root_id not in context.components_by_id
        ):
            reporter.add(
                "error",
                "DSL_ROOT_NOT_FOUND",
        ...
        allowed_template_keys = set(
            template_children_rules.get(
                "allowedKeys", ["componentId", "path", "itemVar", "indexVar"]
            )
        )
        required_template_keys = set(
            template_children_rules.get("requiredKeys", ["componentId", "path"])
        )
        ...
        for index, component in enumerate(context.components):
            ...
            forbidden = sorted(
                (forbidden_global | set(forbidden_by_component.get(component_type, [])))
                & set(component.keys())
            )
            if forbidden:
                reporter.add(
            ...
            if (
                ...
                and component_id == context.root_id
            ):
                ...
                if isinstance(children, list):
                    for child_index, child in enumerate(children):
                        if not isinstance(child, str):
                            reporter.add(
                                ...
                                "genui",
                                line=2,
                                json_pointer=f"{pointer}/children/{child_index}",
                                actual=child,
                                message="普通 children 只能包含组件 id 字符串。",
                            )
                        elif child not in context.components_by_id:
                            reporter.add(
                                "error",
                                "DSL_CHILD_REF_NOT_FOUND",
                                "hard",
                                "genui",
                                line=2,
                                json_pointer=f"{pointer}/children/{child_index}",
                                actual=child,
                                message="children 引用了不存在的组件 id。",
                                fix_hint="新增该 id 的组件，或删除/改正 children 引用。",
                            )
                    ...
                    if component_type not in template_components:
                        reporter.add(
                            ...
                            "genui",
                            line=2,
                            json_pointer=f"{pointer}/children",
                            message=f"{component_type}.children 不支持模板循环对象。",
                        )
                    if (
                        not required_template_keys <= set(children.keys())
                        or set(children.keys()) - allowed_template_keys
                    ):
                        reporter.add(
                            "error",
                            "DSL_TEMPLATE_CHILDREN_INVALID",
                    ...
                    if child_id not in context.components_by_id:
                        reporter.add(
                            ...
                            "genui",
                            line=2,
                            json_pointer=f"{pointer}/children/componentId",
                            actual=child_id,
                            message="模板 componentId 引用了不存在的组件 id。",
                        )
                    if expression_like(child_path) or not is_json_pointer(child_path):
                        reporter.add(
                            ...
                            "genui",
                            line=2,
                            json_pointer=f"{pointer}/children/path",
                            actual=child_path,
                            message="模板 children.path 必须是结构 JSON Pointer，不能是表达式。",
                        )
                    for var_field in ("itemVar", "indexVar"):
                        ...
                        if has_var and is_invalid_var:
                            reporter.add(
                                ...
                                "genui",
                                line=2,
                                json_pointer=f"{pointer}/children/{var_field}",
                                actual=var_name,
                                message=f"{var_field} 必须是不带 $ 前缀的非空变量名。",
                            )
                else:
                    reporter.add(
                        "error",
                        ...
                        json_pointer=f"{pointer}/children",
                        actual=children,
                        message="children 必须是组件 id 数组或模板循环对象。",
                    )
