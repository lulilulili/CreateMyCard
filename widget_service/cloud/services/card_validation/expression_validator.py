...
class ExpressionValidator(BaseValidator):
    ...
    def validate(self, context, rules, reporter) -> None:
        ...
        self._banned_variables = tuple(
            expression_rules.get(
                "bannedVariables", ["$__widthBreakpoint", "$__colorMode", "$context"]
            )
        )
        self._banned_operators = tuple(
            expression_rules.get("bannedOperators", ["===", "!==", "??", "?."])
        )
        self._banned_keywords = tuple(expression_rules.get("bannedKeywords", ["and", "or", "not"]))
    ...
    def _check_expression(self, value: str, pointer: str, line: int | None, reporter) -> None:
        ...
        if len(stripped) > self._max_length:
            reporter.add(
                ...
                line=line,
                json_pointer=pointer,
                message=f"表达式长度不能超过 {self._max_length} 字符。",
            )
        if self._paren_depth(body) > self._max_paren_depth:
            reporter.add(
                ...
                line=line,
                json_pointer=pointer,
                message=f"表达式括号嵌套不能超过 {self._max_paren_depth} 层。",
            )
        ...
        for function_name in self._function_re.findall(body):
            if function_name not in self._allowed_functions:
                reporter.add(
                    ...
                    expected=sorted(self._allowed_functions),
                    message="表达式只允许使用声明的内置函数。",
                )
        for ref in expression_references(stripped):
            if not ref:
                ...
                reporter.add(
                    ...
                    json_pointer=pointer,
                    actual=ref,
                    message="JSON Pointer 片段中出现点号，请确认不是误写成点路径。",
                )
        ...
        for component in context.components:
            ...
            for field in ("id", "component"):
                if expression_like(component.get(field)):
                    reporter.add(
                        ...
                        "genui",
                        line=2,
                        json_pointer=f"/updateComponents/componentsById/{component_id}/{field}",
                        actual=component.get(field),
                        message=f"{field} 不允许表达式。",
                    )
                    ...
                    for field in ("call", "as"):
                        if expression_like(handler.get(field)):
                            reporter.add(
                                ...
                                "genui",
                                line=2,
                                json_pointer=f"/updateComponents/componentsById/{component_id}/onClick/{index}/{field}",
                                actual=handler.get(field),
                                message=f"EventHandler.{field} 不允许表达式。",
                            )
            ...
            if isinstance(children, dict) and expression_like(children.get("path")):
                reporter.add(
                    ...
                    "genui",
                    line=2,
                    json_pointer=f"/updateComponents/componentsById/{component_id}/children/path",
                    actual=children.get("path"),
                    message="模板 children.path 不允许表达式。",
                )
