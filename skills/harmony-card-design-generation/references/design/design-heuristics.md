...
- 一个服务对象和一个主问题、一个主色族；动作目标不超过当前尺寸或模块允许上限。
- `mustKeep` 完整显示，`shouldKeep` 不挤占主显示组、受保护文本或动作热区。
- `2x2` / `2x4` 尺寸和主要区域预算已经成立。
- 非图片颜色能回溯到 token、多彩色或合规场景拓展色。
- 动作能力和素材来源已经声明；不明点击或素材已经删除。

术语兜底：`Design Plan` 是选中 layout、module、element 和 content 的内部对象；`metric/tile/status/badge` 是模块声明的并列小事实槽位。

## 职责边界

- 内容取舍和尺寸适配读 `../generation-contract.md`；布局、模块和风格选择读 `../routing-and-style.md`。
...
- 支撑组只包含 `shouldKeep` 或必要 `mustKeep`；模块内 `metric/tile/status/badge` 按 slots 独立计数，但不得重复 `primary`。
...
- 不使用网络图、内联/base64 图片、占位图或相似路径猜测；只逐字使用素材库声明的本地/资源路径 SVG、PNG。
...
- 支撑组是否只包含已入选 `shouldKeep` 字段？模块里的 `metric/tile/status/badge` 是否来自 slots、互不重复且没有抢占主显示组？
