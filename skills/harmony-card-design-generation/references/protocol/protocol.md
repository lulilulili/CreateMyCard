...
- 默认不要使用自定义组件。只有用户或宿主明确说明 catalog 已注册自定义组件时才可使用；最终只在一句布局/风格说明后输出 `genui` 和 `cardspec` 两个代码块，不额外输出宿主假设说明。
...
- 媒体：网络图片、内联/base64 图片、未声明资源路径、`data:image/*`；素材库明确声明的本地/资源路径 SVG 允许使用
...
- `Image.src` 和 `styles.backgroundImage` 只使用用户提供或素材库逐字声明的本地/资源路径 SVG、PNG；不支持网络 URL、内联/base64 图片，也不得自行拼接、改名或替换扩展名。
