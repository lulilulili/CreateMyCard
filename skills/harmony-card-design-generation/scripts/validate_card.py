...
def read_json_object(path: str | None, object_id: str | None = None) -> dict:
    if not path:
        return {}
    import json
    ...
    if isinstance(value.get("plans"), list):
        if not object_id:
            raise ValueError("--design-plan-id is required when design plan contains a plans array")
        selected = next((item for item in value["plans"] if isinstance(item, dict) and item.get("surfaceId") == object_id), None)
        if selected is None:
            raise ValueError(f"unknown design plan id: {object_id}")
        return selected
    return value
...
def main() -> int:
    ...
    parser.add_argument("--design-plan", help="Path to design-plan-v1 used to assemble the card.")
    parser.add_argument("--design-plan-id", help="Select surfaceId when --design-plan contains a plans array.")
    ...
    design_plan = read_json_object(args.design_plan, args.design_plan_id)
