...
4. 定尺寸与适配方向：用户指定 `2x2` 或 `2x4` 时优先尊重；未指定时按“最小充分尺寸”先验证 `2x2`，删除非核心可选内容后仍无法完整承载核心需求，才把 `best-fit` 判为 `2x4`。
...
`best-fit` 表示能完整满足核心需求的最小尺寸，不表示视觉上最舒展的尺寸。用户未指定时必须先验证 `2x2`；“信息较多”“横版更舒展”“视觉更丰富”或“存在两个数据能力”本身不能把 `best-fit` 判为 `2x4`。
...
- `2x4`：只有 `2x2` 在删除 `shouldKeep` 等非核心可选内容后，仍因受保护文本、必要热区、必须同屏理解的并列关系、关键媒体或整体布局预算无法成立时使用。短时间线最多 3 项；周/小矩阵/媒体预览/显式动作只有确实需要横向空间且属于核心需求时才构成升级理由；最多 2 个热区，且每个热区有明确 `onClick`、标签和不小于 `24vp` 的点击视觉尺寸。
